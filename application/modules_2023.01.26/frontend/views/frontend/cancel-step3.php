<!DOCTYPE html>
<html lang="en">

<head>
    <?php require('inc_header.php'); ?>
    
</head>
<style>
    .cancel_text{
        text-align: right;
        padding-top: 50px;
        font-size: 14px;
    }
    .buttom_cancel a{
        color: #fff;
        border: 1px solid #ec008c;
        background-color: #ec008c;
        display: inline-block;
        line-height: 27px;
        transition: 0.5s;
        font-size: 14px;
        text-align: center;
        width: 45%;
        border-radius: 35px;
    }
    .buttom_cancel{
        text-align: right;
        padding-top: 54px;
    }
    @media (max-width: 991px) {
        .img_member {
            display: none;
        }
    }
</style>
<body>
    <?php require('inc_menu.php'); ?>
        
    <div class="container-fluid">
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <div class="boxtext_help">
                        <h2>My Account</h2>
                    </div>
                    <div class="row pad_account">
                        <div class="col-12 col-md-12 col-lg-3">
                            <?php require('inc_account.php'); ?>
                        </div>
                        <div class="col-12 col-md-12 col-lg-9 bg_payment">
                            <div class="text-myaccount pad_myaccount">
                                <h5>Cancel</h5>
                            </div>
<?php
//pre($cancelCtrl);
if(!empty($cancelCtrl)) {
    foreach($cancelCtrl as $r) {
        /*$order = $this->model_frontend->getCancelOrderResult($r->order_detail_id);
        if(!empty($order)) {
            foreach($order as $c) {*/
                $weight = $this->model_frontend->getWeightRecord($r->order_weight);
                $color = $this->model_frontend->getColorRecord($r->order_color);
                $collection = $this->model_frontend->getCollectionRecord($r->order_collection);
?>
                            <div class="bgreturn">
                                <div class="row">
                                    <div class="col-xs-5 col-sm-6"> <?php echo dateCancelMistine($r->order_detail_datetime_create);?> </div>
                                    <div class="col-xs-7 col-sm-6 right"> ORDER No. <?php echo $r->order_no;?> </div>
                                </div>
                            </div>
                            <div class="row return_product">
                                <div class="col-2">
                                    <img src="<?php echo base_url('uploads/product/'.$r->order_image);?>" class="img-fluid">
                                </div>
                                <div class="col-6">
                                    <h5><?php echo $r->order_name;?></h5>
                                    <ul class="ul_return_product">
                                        <li>Quantity : <?php echo $r->order_qty;?></li>
                                        <li>Weight : <?php if(!empty($weight)) echo $weight->weight_name_lang1; else echo '-';?></li>
                                        <li>Color : <?php if(!empty($color)) echo $color->color_name_lang1; else echo '-';?></li>
                                        <li>Collection : <?php if(!empty($collection)) echo $collection->collection_name_lang1; else echo '-';?></li>
                                    </ul>
                                </div>
                                <div class="col-4">
                                    <h4><!-- <span>$200</span> --> $<?php echo $r->order_price;?></h4>
                                    <!-- <div class="buttom_cancel"><a href="<?php echo site_url('frontend/path/cancel_step2/'.$c->order_id);?>">Cancel</a></div> -->
                                </div>
                            </div>
<?php
            /*}
        }*/
    }
}

$this->db->join('ci_cancel', 'ci_order_detail.order_detail_id = ci_cancel.order_detail_id and ci_order_detail.member_id = ci_cancel.member_id', 'inner');
$this->db->where('ci_order_detail.member_id', $this->session->userdata('member_id'));
$this->db->group_by('ci_order_detail.order_detail_id');
$query_cancel = $this->db->get('ci_order_detail');

$cancelCtrl = $query_cancel->result();

if(!empty($cancelCtrl)) {
    foreach($cancelCtrl as $r) {
?>
                            <div class="bgreturn">
                                <div class="row">
                                    <div class="col-xs-5 col-sm-6"> <?php echo dateCancelMistine($r->order_detail_datetime_create);?> </div>
                                    <div class="col-xs-7 col-sm-6 right"> ORDER No. <?php echo $r->order_no;?> </div>
                                </div>
                            </div>
<?php
        $this->db->where('ci_order_detail.order_detail_id', $r->order_detail_id);
        $this->db->join('ci_order', 'ci_order_detail.order_detail_id = ci_order.order_detail_id', 'inner');
        $query_order = $this->db->get('ci_order_detail');
        $order = $query_order->result();
        
        if(!empty($order)) {
            foreach($order as $c) {
                $weight = $this->model_frontend->getWeightRecord($c->order_weight);
                $color = $this->model_frontend->getColorRecord($c->order_color);
                $collection = $this->model_frontend->getCollectionRecord($c->order_collection);
?>
                            <div class="row return_product">
                                <div class="col-2">
                                    <img src="<?php echo base_url('uploads/product/'.$c->order_image);?>" class="img-fluid">
                                </div>
                                <div class="col-6">
                                    <h5><?php echo $c->order_name;?></h5>
                                    <ul class="ul_return_product">
                                        <li>Quantity : <?php echo $c->order_qty;?></li>
                                        <li>Weight : <?php if(!empty($weight)) echo $weight->weight_name_lang1; else echo '-';?></li>
                                        <li>Color : <?php if(!empty($color)) echo $color->color_name_lang1; else echo '-';?></li>
                                        <li>Collection : <?php if(!empty($collection)) echo $collection->collection_name_lang1; else echo '-';?></li>
                                    </ul>
                                </div>
                                <div class="col-4">
                                    <h4><!-- <span>$200</span> --> $<?php echo $c->order_price;?></h4>
                                    <!-- <div class="buttom_cancel"><a href="<?php echo site_url('frontend/path/cancel_step2/'.$c->order_id);?>">Cancel</a></div> -->
                                </div>
                            </div>
<?php
            }
        }
    }
}
?>
                        </div>
                    </div>

                </div>
            </div>
        </div>
        <?php require('inc_footer.php'); ?>
        <script>
            $('.sidemenumem li:nth-child(7) ').addClass('active');
        </script>
    </div>

</body>

</html>