...
?>					    
							...
							<div class="form-group">
					            ...
					            <div class="col-md-9" style="padding-top: 7.25px;">
                       				<?php if(!empty($row)) echo $row->shipping_location;?>
					            </div>
					        </div>	
							<div class="form-group">
					            <label class="col-md-3 control-label">Township</label>
					            <div class="col-md-9" style="padding-top: 7.25px;">
                       				<?php if(!empty($row)) echo $row->shipping_township;?>
					            </div>
					        </div>	
							<div class="form-group">
							...
							<div class="form-group">
					            <label class="col-md-3 control-label">Document Create</label>
					            <div class="col-md-9" style="padding-top: 7.25px;">
							...
							<div class="form-group">
					            ...
					            <div class="col-md-9" style="padding-top: 7.25px;">
                       				<input type="button" name="send_email" id="send_email" value="Send Email" class="form-control" <?php if(!empty($row) and $row->order_detail_status != 'Delivery') echo 'disabled';?> onclick="sendEmailTrackingNo();">
					            </div>
					        </div>
					        <div class="form-group">
	...
	<script>
		...
		function changeStatusOrder(order_detail_status, order_detail_id) {
			$.post('<?php echo site_url("order/backend/ajaxChangeStatus");?>', { order_detail_status: order_detail_status, order_detail_id: order_detail_id }, function(data) {
				...
				if(data != 'Delivery') {
					$("#order_detail_tracking_no").prop('readonly', true);
					$("#send_email").prop('disabled', true);
				} else {
					$("#order_detail_tracking_no").prop('readonly', false);
					$("#send_email").prop('disabled', false);
				}
			});
		}

		function sendEmailTrackingNo() {
			if($("#order_detail_status").val() == 'Delivery') {
				if($("#order_detail_tracking_no").val() == '') {
					alert('Please enter Tracking No.');

					$("#order_detail_tracking_no").focus();
				} else {
					$("#send_email").hide();
					
					$.post('<?php echo site_url("order/backend/ajaxSendEmailTrackingNo");?>', { order_detail_tracking_no: $("#order_detail_tracking_no").val(), order_detail_id: '<?php echo $id;?>' }, function(data) {
						if(data == true) {
							alert('Send Email Tracking No. Success');
						}
					})
				}
			}
